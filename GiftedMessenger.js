...
class GiftedMessenger extends Component {
  ...
  _handleScroll(event) {
    if (!this.props.autoLoad || !this.props.loadEarlierMessagesButton || this.props.isLoadingEarlierMessages) { return; }

    let n_event = event.nativeEvent;
    if (n_event.contentOffset.y < 30) {
    ...
    if (this.props.hideTextInput === false) {
      return (
        <View style={this.styles.textInputContainer}>
          ...
          />
        {(!this.state.disabled) ? <View style={this.styles.sendButtonContainer}>
            <Button
              style={this.styles.sendButton}
              styleDisabled={this.styles.sendButton}
              onPress={this.onSend}
              disabled={false}
            >
              {this.props.sendButtonText}
            </Button>
          </View> : <View style={this.styles.sendButtonContainerDisabled}>
            <Button
              style={this.styles.sendButtonDisabled}
